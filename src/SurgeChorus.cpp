...
struct SurgeChorusWidget : SurgeModuleWidgetCommon {
    ...
    void moduleBackground(NVGcontext *vg) {
        ...
        {
            centerRuledLabel(vg, padFromEdge, regionStartY(i), boxw - 2 * padFromEdge, regionLabels[i].c_str() );
        }
...
SurgeChorusWidget::SurgeChorusWidget(SurgeChorusWidget::M *module)
    : SurgeModuleWidgetCommon()
{
    ...
    for( int i=0; i<2; ++i )
    {
        ...
        addParam(rack::createParam<SurgeSmallKnob>(rack::Vec(cb.pos.x + extraOff, cb.pos.y + extraY),
                                                   module, M::FX_PARAM_0 + paramId[i]));
        addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(cb.pos.x + extraOff + portX + padMargin, cb.pos.y + extraY),
                                                     module, M::FX_PARAM_INPUT_0 + paramId[i]));
        if( i == 0 )
        {
            addParam(rack::createParam<SurgeSwitch>(rack::Vec(cb.pos.x + extraOff + 2 * portX + 2 * padMargin, cb.pos.y + 2 + extraY),
        ...
        addChild(TextDisplayLight::create(
                     rack::Vec(boxw - padFromEdge - upperTextWidth + padMargin,
    ...
    for( int i=2; i<8; ++i )
    {
        auto cb = controlBox(i);
        float extraOff = 5;
        addParam(rack::createParam<SurgeSmallKnob>(rack::Vec(cb.pos.x + lowerCtrlOff + extraOff, cb.pos.y),
                                                   module, M::FX_PARAM_0 + paramId[i]));
        addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(cb.pos.x + lowerCtrlOff + extraOff + portX + padMargin, cb.pos.y),
                                                     module, M::FX_PARAM_INPUT_0 + paramId[i]));
        addChild(TextDisplayLight::create(
                     rack::Vec(cb.pos.x + lowerCtrlOff + 2,
                               cb.pos.y + cb.size.y - textHeight ),
    ...
    addInput(rack::createInput<rack::PJ301MPort>(
                 p0.pos, module, M::CLOCK_CV_INPUT ) );

}

auto mchorus = addFX(
    rack::createModel<SurgeChorusWidget::M, SurgeChorusWidget>("SurgeChorus"), fxt_chorus4 );
