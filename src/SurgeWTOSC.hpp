...
struct SurgeWTOSC : virtual public SurgeModuleCommon
{
    ...
    void clear_osc(int c)
    {
        if (surge_osc[c])
        {
            surge_osc[c]->~Oscillator();
            surge_osc[c] = nullptr;
        }
    }
    ...
    virtual void setupSurge()
    {
        ...
        /*
        ** FIXME: This is a foul hack we should work around. Something about the
        *surge synth
        ** initialization path sets up wavetables in a way which means you can
        *load any wt
        ** you want, but in surge-rack if you haven't loadeed a 512 wide
        *wavetable once
        ** you get odd behavior. See issue #124.
        **
        ** I should really fix that in surge proper but to get shipping for now,
        *I'm just
        ** going to do the gross thing of finding "sawtooth reso" in our list
        *and loading that at
        ...
        */
        ...
        {
            clear_osc(c);
            surge_osc[c] = spawn_osc(ot_wavetable, storage.get(), oscstorage,
        ...
        ** as long as noone changes the memory layout in SurgeStorage.h. Namely
        *that
        ** in oscstorage all the parameters are "up front" and are in order with
        ** nothing between them. With 1.6.3 that broke hence the expliit start
        *point
        */
    ...
    void updateWtLabels()
    {
        ...
        char txt[256];
        snprintf(txt, 256, "%d tbl of %d samples", oscstorage->wt.n_tables, oscstorage->wt.size);
    ...
    void updateWtIdx()
    {
        ...
        */
        ...
        if (patchIdx == cat.numberOfPatchesInCatgory)
            patchIdx--;
    ...
    void process(const typename rack::Module::ProcessArgs &args) override
    {
        ...
        {
            /*
            ** re-init new voices so they can get all the latest settings like
            *unison
            */
        ...
        if (processPosition >= BLOCK_SIZE)
        {
            ...
            if (pc.changed(PITCH_0, this) || pc.changed(PITCH_0_IN_FREQ, this))
            {
                if (pc.changed(PITCH_0_IN_FREQ, this) && getParam(PITCH_0_IN_FREQ) > 0.5)
                {
                    // We have switched from note to frequency. We really want
                    // to be at the note exactly
            ...
            {
                knobSaver.storeParams((int)pc.get(WT_OR_WINDOW), OSC_CTRL_PARAM_0,
                                      OSC_CTRL_PARAM_0 + n_osc_params, this);
                ...
                if (firstRespawnIsFromJSON)
                {
                    ...
                    {
                        clear_osc(c);
                        surge_osc[c] = spawn_osc(toWhat, storage.get(), oscstorage,
                                                 storage->getPatch().scenedata[0], oscbuffer[c]);
                        surge_osc[c]->init(72.0);
                        surge_osc[c]->init_ctrltypes();
                        for (int i = 0; i < n_osc_params; ++i)
                        {
                            ...
                            {
                                oscstorage->p[i].deactivated =
                                    !getParam(OSC_DEACTIVATE_INVERSE_PARAM_0 + i);
                            }
            ...
            if (forceRespawnDueToSampleRate || pc.changedAndIsNonZero(LOAD_WT, this) ||
                firstRespawnIsFromJSON)
            {
                oscstorage->wt.queue_id = wtIdx;
                storage->perform_queued_wtloads();
                for (int c = 0; c < nChan; ++c)
            ...
            if (outputConnected(OUTPUT_L) || outputConnected(OUTPUT_R))
            {
                for (int c = 0; c < nChan; ++c)
                {
                    ...
                    copyScenedataSubset(0, storage_id_start, storage_id_end);
                    float pitch0 = (getParam(PITCH_0_IN_FREQ) > 0.5) ? getParam(PITCH_0)
                                                                     : (int)getParam(PITCH_0);
                    if (initBeforeProcess)
                    {
                        surge_osc[c]->init(pitch0);
                    }

                    surge_osc[c]->process_block(pitch0 + inputs[PITCH_CV].getVoltage(c) * 12.0, 0,
                                                true);
        ...
        for (int c = 0; c < nChan; ++c)
        {
            float avgl = (surge_osc[c]->output[processPosition] +
                          surge_osc[c]->output[processPosition + 1]) *
                         0.5;
            float avgr = (surge_osc[c]->outputR[processPosition] +
                          surge_osc[c]->outputR[processPosition + 1]) *
                         0.5;
            if (outputConnected(OUTPUT_L) && !outputConnected(OUTPUT_R))
            {
                // Special mono mode
    ...
    virtual void dataFromJson(json_t *rootJ) override
    {
        ...
        if (jcat && jwt)
        {
            ...
            if (storage->wt_category.size() == 0)
            {
                WARN("Found no wavetables present in dataFromJSON; ignoring "
                     "saved state");
                return;
            }
            ...
            params[WT_IN_CATEGORY_IDX].setValue(
                idxInCat * 1.0 /
                storage->wt_category[storage->wt_list[newIdx].category].numberOfPatchesInCatgory);
