...
struct SurgeLFOWidget : SurgeModuleWidgetCommon
{
    ...
    float inputYPos(int which) { return orangeLine + ioMargin + padMargin; }
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        drawBlueIORect(vg, xb0, ioyb, 3 * portX + 6 * padMargin,
        ...
        drawTextBGRect(vg, box.size.x / 2 + padMargin, boxLayerY0 + labelH + 2,
                       box.size.x / 2 - 2 * padMargin, portY - 2);
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            drawTextBGRect(vg, xpos, ypos + labelH + portY + 2 * padMargin,
                           box.size.x / 3 - 2 * padFromEdge, 14);
        }
...
SurgeLFOWidget::SurgeLFOWidget(SurgeLFOWidget::M *module) : SurgeModuleWidgetCommon()
{
    setModule(module);
    ...
    addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(inputXPos(0), inputYPos(0)), module,
                                                 M::GATE_IN));
    addOutput(rack::createOutput<rack::PJ301MPort>(rack::Vec(inputXPos(2), inputYPos(2)), module,
                                                   M::OUTPUT_ENV));
    ...
    addParam(rack::createParam<SurgeSwitchFull>(
        rack::Vec(padFromEdge + padMargin + buttonBankW,
    ...
    addInput(
        rack::createInput<rack::PJ301MPort>(rack::Vec(xPos, rateTop), module, M::CLOCK_CV_INPUT));
    xPos += portX + padMargin + clockPad + 18;
    addParam(rack::createParam<SurgeSmallKnob>(rack::Vec(xPos, rateTop), module, M::RATE_PARAM));
    xPos += portX + padMargin;
    addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(xPos, rateTop), module, M::RATE_CV));

    xPos += portX + padMargin;
    addParam(rack::createParam<SurgeSwitch>(rack::Vec(xPos, rateTop + 2), module, M::RATE_TS));

    addChild(TextDisplayLight::create(rack::Vec(box.size.x / 2 + 2 * padMargin, rateTop),
                                      rack::Vec(box.size.x / 2 - 3 * padMargin, portY - 2),
                                      module ? &(module->pb[0]->valCache) : nullptr, 14,
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        addParam(rack::createParam<SurgeSmallKnob>(rack::Vec(xc0, ypos), module,
                                                   M::RATE_PARAM + mpd[i]));
        addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(xc0 + portX + padMargin, ypos),
                                                     module, M::RATE_CV + mpd[i]));

        addChild(TextDisplayLight::create(rack::Vec(xpos, ypos - 2 + portY + 2 * padMargin),
                                          rack::Vec(box.size.x / 3 - 2 * padFromEdge, 14),
                                          module ? &(module->pb[mpd[i]]->valCache) : nullptr, 12,
                                          NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE,
                                          parameterValueText_KEY()));
    }

    addChild(rack::createLight<rack::SmallLight<rack::BlueLight>>(
        rack::Vec(box.size.x / 2 + 25, envPos), module, M::ENV_LIGHT));
    ...
    for (int i = 7; i < nControls; ++i)
    {
        ...
        addParam(
            rack::createParam<SurgeSmallKnob>(rack::Vec(xstart + padMargin, ep.y), module, pa));
        if (pa != M::S_PARAM)
        {
            addParam(rack::createParam<SurgeSwitch>(
                rack::Vec(xstart + 2 * portX + 3 * padMargin, ep.y), module, ts));
        }

        addChild(TextDisplayLight::create(
            rack::Vec(xstart + padMargin, ep.y + portY + padMargin),
            rack::Vec(textAreaWidth - 2 * padMargin, controlHeight - padMargin),
            module ? &(module->pb[idx]->valCache) : nullptr, 14, NVG_ALIGN_LEFT | NVG_ALIGN_TOP,
            parameterValueText_KEY()));
    }
}
