...
void SurgeRackParamBinding::updateFloat(const ParamCache &pc, int polyChannel, SurgeModuleCommon *m)
{
    ...
    if (paramChanged || forceRefresh || (cv_id >= 0 && m->inputConnected(cv_id)))
        p->set_value_f01(m->getParam(param_id) +
                         m->inputs[cv_id].getPolyVoltage(polyChannel) / 10.0);
}

void SurgeRackParamBinding::updateBool(const ParamCache &pc, int polyChannel, SurgeModuleCommon *m,
                                       bool notIt)
{
...
std::string SurgeRackParamQuantity::getLabel()
{
    ...
    if (mc)
    {
        ...
        if (ts_companion == -2)
        {
            ...
            if (ts_companion == -2)
                ts_companion = -1; // only check once
        }
        if (ts_companion >= 0)
        {
            char txt[1024];
