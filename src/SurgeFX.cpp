...
template <int effectType> struct SurgeFXWidget : SurgeModuleWidgetCommon
{
    ...
    : SurgeModuleWidgetCommon()
{
    ...
    addParam(rack::createParam<SurgeSmallKnob>(ioPortLocation(false, 2, box), module, M::OUTPUT_GAIN

                                               ));

    addInput(rack::createInput<rack::PJ301MPort>(
        rack::Vec(box.size.x / 2 - portX / 2, orangeLine + 1.5 * ioMargin), module,
        M::CLOCK_CV_INPUT));
    ...
    for (int i = 0; i < nControls; ++i)
    {
        ...
        {
            addParam(rack::createParam<SurgeSwitch>(rack::Vec(3 * padMargin + 2 * portX, yPos),
                                                    module, M::PARAM_TEMPOSYNC_0 + i));
        }
...
#define CREATE_FX(type, name)                                                                      \
    auto v##type =                                                                                 \
        addFX(rack::createModel<SurgeFXWidget<type>::M, SurgeFXWidget<type>>(name), type);
