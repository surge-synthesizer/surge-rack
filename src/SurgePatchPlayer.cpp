...
struct SurgePatchPlayerWidget : public virtual SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        yTPos += 19 + padMargin;
        drawTextBGRect(vg, xTPos, yTPos, box.size.x - padFromEdge - xTPos,
                       orangeLine - yTPos - padMargin);
    }
};
    ...
    : SurgeModuleWidgetCommon()
{
    ...
    addInput(rack::createInput<rack::PJ301MPort>(ioPortLocation(true, 0, box), module, M::INPUT_L));
    addInput(rack::createInput<rack::PJ301MPort>(ioPortLocation(true, 1, box), module, M::INPUT_R));
    addParam(
        rack::createParam<SurgeSmallKnob>(ioPortLocation(true, 2, box), module, M::INPUT_GAIN));

    addOutput(
        rack::createOutput<rack::PJ301MPort>(ioPortLocation(false, 0, box), module, M::OUTPUT_L));
    addOutput(
        rack::createOutput<rack::PJ301MPort>(ioPortLocation(false, 1, box), module, M::OUTPUT_R));
    addParam(
        rack::createParam<SurgeSmallKnob>(ioPortLocation(false, 2, box), module, M::OUTPUT_GAIN));

    addInput(rack::createInput<rack::PJ301MPort>(
        rack::Vec(box.size.x / 2 - portX / 2, orangeLine + 1.5 * ioMargin), module, M::CLOCK_CV));

    addChild(TextDisplayLight::create(rack::Vec(padMargin, SCREW_WIDTH + padMargin),
                                      rack::Vec(box.size.x - 2 * padMargin, 15),
                                      module ? &(module->patchInfoCache[0]) : nullptr, 13,
                                      NVG_ALIGN_TOP | NVG_ALIGN_CENTER, parameterValueText_KEY()));

    addChild(TextDisplayLight::create(rack::Vec(padMargin, SCREW_WIDTH + padMargin + 13),
                                      rack::Vec(box.size.x - 2 * padMargin, 25),
                                      module ? &(module->patchInfoCache[1]) : nullptr, 20,
                                      NVG_ALIGN_TOP | NVG_ALIGN_CENTER, parameterValueText_KEY()));

    addChild(TextDisplayLight::create(rack::Vec(padMargin, SCREW_WIDTH + padMargin + 13 + 2 + 20),
                                      rack::Vec(box.size.x - 2 * padMargin, 15),
                                      module ? &(module->patchInfoCache[2]) : nullptr, 11,
                                      NVG_ALIGN_TOP | NVG_ALIGN_CENTER, parameterValueText_KEY()));
    ...
    for (auto tup : ctrlPositions)
    {
        ...
        addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(xc + w / 2 - portX / 2, yc + 14),
                                                     module, ct));
    }

    addChild(TextDisplayLight::create(
        rack::Vec(xTPos + 4, yTPos), rack::Vec(box.size.x - xTPos - 2 * padMargin, 19),
        module ? &(module->patchCategoryName) : nullptr, 15, NVG_ALIGN_MIDDLE | NVG_ALIGN_CENTER,
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        case 0:
            addParam(
                rack::createParam<SurgeKnobRooster>(rack::Vec(xlp, ylp), module, M::CATEGORY_IDX));
            break;
        case 1:
            addParam(rack::createParam<SurgeKnobRooster>(rack::Vec(xlp, ylp), module,
                                                         M::PATCH_IN_CATEGORY_IDX));
            break;
        case 2:
            addParam(rack::createParam<rack::CKD6>(rack::Vec(xlp, ylp), module, M::LOAD_PATCH));
            addChild(rack::createLight<rack::SmallLight<rack::RedLight>>(
                rack::Vec(xlp + 27, ylp + 1), module, M::NEEDS_LOAD));
    ...
    for (int i = 1; i < 6; ++i)
    {
        ...
        auto c = TextDisplayLight::create(
            ...
            module ? &(module->patchItemName[i]) : nullptr, fs,
