...
struct SurgeFreqShiftWidget : SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            drawTextBGRect(vg, xText, y0 + portY + 1.5 * padMargin,
                           box.size.x - xText - padFromEdge, textHeight);
            y0 += ctrlHeight;
        }
    ...
    : SurgeModuleWidgetCommon()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        addChild(
            TextDisplayLight::create(rack::Vec(xText + padMargin, yp + portY + 1.5 * padMargin),
...
auto mfreq =
    addFX(rack::createModel<SurgeFreqShiftWidget::M, SurgeFreqShiftWidget>("SurgeFreqShift"),
          fxt_freqshift);
