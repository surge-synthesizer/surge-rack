...
struct SurgeOSC : virtual public SurgeModuleCommon
{
    ...
    virtual void setupSurge()
    {
        ...
        oscConfigurations.push_back(std::pair<int, std::string>(ot_FM2, "FM2 (fixed)"));
        oscConfigurations.push_back(std::pair<int, std::string>(ot_FM3, "FM3 (free)"));
        ...
        ** nothing between them. With 1.6.3 I broke that assumptino and just
        *assumeh tye
        ** are contiguous
        */
    ...
    void respawn(int i, int idx)
    {
        ...
        surge_osc[idx] = spawn_osc(i, storage.get(), oscstorage, storage->getPatch().scenedata[0],
                                   oscbuffer[idx]);
    ...
    void process(const typename rack::Module::ProcessArgs &args) override
    {
        ...
        if (processPosition >= BLOCK_SIZE)
        {
            ...
            {
                knobSaver.storeParams((int)pc.get(OSC_TYPE), OSC_CTRL_PARAM_0,
            ...
            for (int i = 0; i < n_osc_params; ++i)
            {
                ...
                if (getParam(OSC_CTRL_PARAM_0 + i) != pc.get(OSC_CTRL_PARAM_0 + i) || respawned)
                {
                    oscstorage->p[i].set_value_f01(getParam(OSC_CTRL_PARAM_0 + i));
                    resetCache = true;
                }
            ...
            for (int c = 0; c < nChan; ++c)
            {
                ...
                ** voice here; so in classic mode if unison changes, then go
                *ahead
                ** and re-init
                */
                if (newUnison)
                ...
                if (outputConnected(OUTPUT_L) || outputConnected(OUTPUT_R))
                {
                    for (int i = 0; i < n_osc_params; ++i)
                    {
                        ...
                        {
                            oscstorage->p[i].set_value_f01(
                                getParam(OSC_CTRL_PARAM_0 + i) +
