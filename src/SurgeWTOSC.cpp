...
struct SurgeWTOSCWidget : public virtual SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        centerRuledLabel(vg, colOneEnd + padFromEdge, infoPos0,
                         box.size.x - colOneEnd - 2 * padFromEdge, "Current WT");
        ...
        drawTextBGRect(vg, colOneEnd + padFromEdge, yTPos, box.size.x - colOneEnd - 2 * padFromEdge,
                       20);
        yTPos += 20 + padMargin;
        drawTextBGRect(vg, colOneEnd + padFromEdge, yTPos, box.size.x - colOneEnd - 2 * padFromEdge,
                       124);
...
SurgeWTOSCWidget::SurgeWTOSCWidget(SurgeWTOSCWidget::M *module) : SurgeModuleWidgetCommon()
{
    setModule(module);
    ...
    int xp = pitchCtrlX;
    addParam(rack::createParam<SurgeKnobRooster>(rack::Vec(xp, pitchY), module, M::PITCH_0));
    addInput(rack::createInput<rack::PJ301MPort>(
        rack::Vec(xp + surgeRoosterX + padMargin, pitchY + (surgeRoosterY - portY) / 2), module,
        M::PITCH_CV));
    addParam(rack::createParam<SurgeSwitch>(
        rack::Vec(xp + surgeRoosterX + portX + 2 * padMargin, pitchY + (surgeRoosterY - 21) / 2),
        module, M::PITCH_0_IN_FREQ));

    addChild(TextDisplayLight::create(rack::Vec(pitchCtrlX + 2, pitchY + surgeRoosterY + padMargin),
                                      rack::Vec(surgeRoosterX + portX + 2 * padMargin + 14, portY),
                                      module ? &(module->pitch0DisplayCache) : nullptr, 14,
                                      NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE, parameterValueText_KEY()));
    ...
    for (int i = 0; i < n_osc_params; ++i)
    {
        ...
        addInput(rack::createInput<rack::PJ301MPort>(
            rack::Vec(padFromEdge + padMargin + portX, yctrl), module, M::OSC_CTRL_CV_0 + i));

        addParam(rack::createParam<SurgeDisableStateSwitch>(
            rack::Vec(padFromEdge + 2 * padMargin + 2 * portX, yctrl), module,
            M::OSC_EXTEND_PARAM_0 + i));
        addParam(rack::createParam<SurgeDisableStateSwitch>(
            rack::Vec(padFromEdge + 2 * padMargin + 2 * portX + 12, yctrl), module,
            M::OSC_DEACTIVATE_INVERSE_PARAM_0 + i));

        float xt = padFromEdge + 2 * padMargin + 2 * portX;

        addChild(TextDisplayLight::create(
            rack::Vec(xt + 2 + 22, yp + 0.5),
            rack::Vec(colOneEnd - xt - padMargin, controlHeightPer - padMargin - 2),
            module ? &(module->paramNameCache[i]) : nullptr, 12));
        addChild(TextDisplayLight::create(
            rack::Vec(xt + 2 + 22, yp + 1),
            rack::Vec(colOneEnd - xt - padMargin, controlHeightPer - padMargin - 2),
            module ? (&module->paramValueCache[i]) : nullptr, 15, NVG_ALIGN_LEFT | NVG_ALIGN_BOTTOM,
            parameterValueText_KEY()));
    }

    float c2w = (box.size.x - colOneEnd) / 2 + colOneEnd;
    SurgeButtonBank *bank = SurgeButtonBank::create(
        rack::Vec(c2w - 100 / 2, pitchY), rack::Vec(100, 25), module, M::WT_OR_WINDOW, 2, 1, 13);
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        addChild(TextDisplayLight::create(
            rack::Vec(xp, yp), rack::Vec(box.size.x - colOneEnd - 2 * padMargin, 15),
            module ? &(module->wtInfoCache[i]) : nullptr, 13, NVG_ALIGN_MIDDLE | NVG_ALIGN_LEFT,
            parameterValueText_KEY()));
    }

    auto xpf = [this](int i) {
        return this->colOneEnd + this->catitlodsz * (i + 0.5) - this->surgeRoosterX / 2.0;
    };

    addParam(
        rack::createParam<SurgeKnobRooster>(rack::Vec(xpf(0), wtSelY), module, M::CATEGORY_IDX));

    addParam(rack::createParam<SurgeKnobRooster>(rack::Vec(xpf(1), wtSelY), module,
                                                 M::WT_IN_CATEGORY_IDX));

    addParam(rack::createParam<rack::CKD6>(rack::Vec(xpf(2), wtSelY + 4), module, M::LOAD_WT));

    addChild(rack::createLight<rack::SmallLight<rack::RedLight>>(rack::Vec(xpf(2) + 27, wtSelY + 1),
                                                                 module, M::NEEDS_LOAD));

    auto xTPos = colOneEnd + padMargin;
    auto yTPos = wtSelY + surgeRoosterY + padMargin;
    addChild(TextDisplayLight::create(
        rack::Vec(xTPos + 4, yTPos), rack::Vec(box.size.x - xTPos - 2 * padMargin, 19),
        module ? &(module->wtCategoryName) : nullptr, 15, NVG_ALIGN_MIDDLE | NVG_ALIGN_CENTER,
        parameterValueText_KEY()));
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        auto c = TextDisplayLight::create(
            ...
            module ? &(module->wtItemName[i]) : nullptr, fs, NVG_ALIGN_MIDDLE | NVG_ALIGN_CENTER);
        c->colorKey = "";
        c->color = cl();
