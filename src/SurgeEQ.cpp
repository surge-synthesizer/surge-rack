...
struct SurgeEQWidget : SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        for (int i = 0; i < nBands; ++i)
        {
            ...
            centerRuledLabel(vg, bandRegion * i + padFromEdge, bandY0, bandRegion - 2 * padFromEdge,
                             txt);
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                drawTextBGRect(vg, i * bandRegion + padFromEdge, yPos, bandRegion - 2 * padFromEdge,
                               textHeight);
            }
        }

        centerRuledLabel(vg, box.size.x / 4, masterGainY, box.size.x / 2, "Master Gain");
        drawTextBGRect(vg, box.size.x / 4 + 2 * portX + 2 * padMargin,
                       masterGainY + labelHeight + padMargin + (portY - textHeight) / 2,
                       box.size.x / 2 - 2 * portX - 2 * padMargin, textHeight);
    }
};

SurgeEQWidget::SurgeEQWidget(SurgeEQWidget::M *module) : SurgeModuleWidgetCommon()
{
    setModule(module);
    ...
    for (int i = 0; i < nBands; ++i)
    {
        for (int j = 0; j < 3; ++j)
        {
            ...
            addParam(rack::createParam<SurgeSmallKnob>(
                rack::Vec(xPosCtr - portY - padMargin / 2, yPos), module, pa));
            ...
            addChild(TextDisplayLight::create(rack::Vec(i * bandRegion + padFromEdge, yPos),
                                              ...
                                              12, NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE,
                                              parameterValueText_KEY()));
        }
    }

    addParam(rack::createParam<SurgeSmallKnob>(
        rack::Vec(box.size.x / 4, masterGainY + labelHeight + padMargin), module,
        M::FX_PARAM_0 + 9));
    addInput(rack::createInput<rack::PJ301MPort>(
        rack::Vec(box.size.x / 4 + portX + padMargin, masterGainY + labelHeight + padMargin),
        module, M::FX_PARAM_INPUT_0 + 9));
    addChild(TextDisplayLight::create(
        rack::Vec(box.size.x / 4 + 2 * portX + 3 * padMargin,
                  masterGainY + labelHeight + padMargin + (portY - textHeight) / 2),
        rack::Vec(box.size.x / 2 - 2 * portX - 3 * padMargin, textHeight),
        module ? &(module->pb[9]->valCache) : nullptr, 12, NVG_ALIGN_MIDDLE | NVG_ALIGN_LEFT,
        parameterValueText_KEY()));
}

auto meq = addFX(rack::createModel<SurgeEQWidget::M, SurgeEQWidget>("SurgeEQ"), fxt_eq);
