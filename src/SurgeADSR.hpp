...
struct SurgeADSR : virtual public XTModule
{
    ...
    virtual void setupSurge()
    {
        ...
        for (int i = 0; i < MAX_POLY; ++i)
        {
            ...
            surge_envelopes[i]->init(storage.get(), adsrstorage, storage->getPatch().scenedata[0],
                                     nullptr);
        }
        ...
        */
        ...
        for (int i = A_PARAM; i <= R_PARAM; ++i)
        {
            ...
            pb[i] = std::shared_ptr<SurgeRackParamBinding>(
                new SurgeRackParamBinding(p0, i, A_CV + (i - A_PARAM)));
        ...
        p0 = &(adsrstorage->mode);
        pb[MODE_PARAM] = std::shared_ptr<SurgeRackParamBinding>(
            new SurgeRackParamBinding(SurgeRackParamBinding::BOOL_NOT, p0, MODE_PARAM));

        p0 = &(adsrstorage->a_s);
        for (int i = A_S_PARAM; i < R_S_PARAM; ++i)
        {
            pb[i] = std::shared_ptr<SurgeRackParamBinding>(
                new SurgeRackParamBinding(SurgeRackParamBinding::INT, p0, i));
            p0++;
        }

        wasGated.resize(MAX_POLY);
        std::fill(wasGated.begin(), wasGated.end(), false);
        everGated.resize(MAX_POLY);
        std::fill(everGated.begin(), everGated.end(), false);
        lastStep = BLOCK_SIZE;
    ...
    void process(const typename rack::Module::ProcessArgs &args) override
    {
        ...
        if (lastStep == 0)
        {
            ...
            for (int i = 0; i < nChan; ++i)
            {
                ...
                if (everGated[i])
                {
                    ...
                    if (inNewAttack)
                    {
                        // This is infrequent so do the painful direct
                        // addressing
        ...
        for (int i = 0; i < nChan; i += 4)
        {
            rack::simd::float_4 outputI =
                (output0[i / 4] * (1.0 - frac) + output1[i / 4] * frac) * 10.0;
