...
SurgeOSCWidget::SurgeOSCWidget(SurgeOSCWidget::M *module) : SurgeModuleWidgetCommon()
{
    setModule(module);
    ...
    SurgeButtonBank *bank = SurgeButtonBank::create(
        rack::Vec(padFromEdge, SCREW_WIDTH + padMargin),
        rack::Vec(box.size.x - 2 * padFromEdge, buttonBankHeight), module, M::OSC_TYPE, 5, 1, 13);
    ...
    addChild(TextDisplayLight::create(
        rack::Vec(xp + surgeRoosterX + portX + 3 * padMargin + 20, pitchY + 3),
        rack::Vec(50, surgeRoosterY - 8), module ? &(module->pitch0DisplayCache) : nullptr, 14,
    ...
    for (int i = 0; i < n_osc_params; ++i)
    {
        ...
        addChild(TextDisplayLight::create(
            rack::Vec(xt + 2 + 12, yp + 0.5),
            rack::Vec(box.size.x - xt - padMargin, controlHeightPer - padMargin - 2),
            module ? &(module->paramNameCache[i]) : nullptr, 12));
        addChild(TextDisplayLight::create(
            rack::Vec(xt + 2 + 12, yp + 1),
            rack::Vec(box.size.x - xt - padMargin, controlHeightPer - padMargin - 2),
...
rack::Model *modelSurgeOSC = rack::createModel<SurgeOSCWidget::M, SurgeOSCWidget>("SurgeOSC");
