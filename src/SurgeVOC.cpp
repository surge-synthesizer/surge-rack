...
template <int effectType> struct SurgeVOCWidget : SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        std::vector<std::string> lab = {"Gain / Level", "Gate / Level", "Rate / Filter",
                                        "Q / Filter"};
        rack::Rect bb = box;
        ...
        for (int i = 0; i < nControls; ++i)
        {
            ...
            SurgeStyle::drawTextBGRect(vg, 3 * padMargin + 2 * portX,
                                       i * controlHeight + SCREW_WIDTH + padMargin + controlOffset,
                                       textAreaWidth, textAreaHeight);
        }

#if EXPOSE_163_PARMS
        std::vector<std::string> lab2 = {"# Bands", "Low Band", "High Band", "Mod Xpand",
                                         "Mod Ctr"};
        for (int i = 0; i < lab2.size(); ++i)
        {
            ...
            fxGroupLabel(vg, off + i * controlHeight + SCREW_WIDTH + 2 * padMargin, lab2[i].c_str(),
                         bb);
            SurgeStyle::drawTextBGRect(vg, 3 * padMargin + 2 * portX,
                                       i * controlHeight + SCREW_WIDTH + padMargin + controlOffset,
                                       textAreaWidth, textAreaHeight);
        ...
        drawBlueIORect(vg, box.size.x / 2 - ioRegionWidth / 2, modY0, ioRegionWidth, boxHt);
    ...
    : SurgeModuleWidgetCommon()
{
...
#endif
    SurgeRackBG *bg =
        new SurgeRackBG(rack::Vec(0, 0), box.size, SurgeFXName<effectType>::getName());
    ...
    v.y -= boxHt + ioMargin;
    addInput(rack::createInput<rack::PJ301MPort>(v, module, M::MODULATOR_L_OR_MONO));
    v = ioPortLocation(true, 1, box, true);
    v.y -= boxHt + ioMargin;
    addInput(rack::createInput<rack::PJ301MPort>(v, module, M::MODULATOR_R));

    v = ioPortLocation(true, 2, box, true);
    v.y -= boxHt + ioMargin;
    addParam(rack::createParam<SurgeSmallKnob>(v, module, M::MODULATOR_GAIN));
    ...
    for (int i = 0; i < nControls; ++i)
    {
        ...
        addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(2 * padMargin + portX, yPos), module,
                                                     cv));
        addParam(rack::createParam<SurgeSmallKnob>(rack::Vec(padMargin, yPos), module, pa));

        int tx = 3 * padMargin + 2 * portX + 2;
        addChild(TextDisplayLight::create(
            rack::Vec(tx, yPos), rack::Vec(textAreaWidth - 2 * padMargin, textAreaHeight),
            module ? &(module->pb[pa]->valCache) : nullptr, 14, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE,
            parameterValueText_KEY()));
    }
}

#define CREATE_FX(type, name)                                                                      \
    auto v##type =                                                                                 \
        addFX(rack::createModel<SurgeVOCWidget<type>::M, SurgeVOCWidget<type>>(name), type);
