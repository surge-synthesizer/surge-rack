...
SurgeBiquadWidget::SurgeBiquadWidget(SurgeBiquadWidget::M *module) : SurgeModuleWidgetCommon()
{
    setModule(module);
    ...
    SurgeButtonBank *bank =
        SurgeButtonBank::create(rack::Vec((box.size.x - bankW) / 2.0, SCREW_WIDTH + padMargin),
                                rack::Vec(bankW, bankH), module, M::FILTER_TYPE, 3, 3, 13);
    bank->addLabel("LP");
    bank->addLabel("LP2B");
    bank->addLabel("HP");
    bank->addLabel("BP");
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        addInput(rack::createInput<rack::PJ301MPort>(
            rack::Vec(b.pos.x + ctrlKnobOff + padMargin + portX, b.pos.y), module, M::FREQ_CV + i));

        addChild(TextDisplayLight::create(
            rack::Vec(b.pos.x + textOff + 2, b.pos.y + b.size.y - textHeight),
            rack::Vec(b.size.x - textOff - 4, textHeight),
            module ? &(module->pStrings[i]) : nullptr, 12, NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE,
