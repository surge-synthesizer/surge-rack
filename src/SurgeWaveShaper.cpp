...
struct SurgeWaveShaperWidget : SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        drawTextBGRect(vg, padFromEdge, yPos, box.size.x - 2 * padFromEdge, textArea);
        yPos += padMargin;
    ...
    : SurgeModuleWidgetCommon()
{
    ...
    SurgeButtonBank *bank = SurgeButtonBank::create(
        rack::Vec(padMargin, yPos), rack::Vec(box.size.x - 2 * padMargin, M::n_ws_types * 23),
        module, M::MODE_PARAM, 1, M::n_ws_types, 13);
    ...
    addParam(rack::createParam<SurgeKnobRooster>(
        rack::Vec(box.size.x / 2 - surgeRoosterX / 2, yPos), module, M::DRIVE_PARAM));
    ...
    addChild(TextDisplayLight::create(
        rack::Vec(padFromEdge, yPos + 2), rack::Vec(box.size.x - 2 * padFromEdge, textArea),
        module ? module->dbGainCache.getValue : []() { return std::string("null"); },
        module ? module->dbGainCache.getDirty : []() { return false; }, 10,
        NVG_ALIGN_MIDDLE | NVG_ALIGN_CENTER, parameterValueText_KEY()));
