...
struct SurgeADSRWidget : SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        drawBlueIORect(vg, padFromEdge, ioyb, box.size.x - 2 * padFromEdge,
        ...
        nvgBeginPath(vg);
        nvgText(vg, inputXPos(1) + portX / 2, iotxt, "Retrig", NULL);
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            drawTextBGRect(vg, padFromEdge * 2 + 12, ADSRYPos(i) + 1,
                           box.size.x - padFromEdge * 3 - 20, adsrTextH);

            dropRightLine(vg, padFromEdge + 5, ADSRYPos(i) + 20 + padMargin, (i == 2) ? 55 : 95,
                          ADSRYPos(i) + adsrTextH + padMargin + 3 + portY / 2);
        }
...
SurgeADSRWidget::SurgeADSRWidget(SurgeADSRWidget::M *module) : SurgeModuleWidgetCommon()
{
    setModule(module);
    ...
    addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(inputXPos(0), inputYPos(0)), module,
                                                 M::GATE_IN));
    addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(inputXPos(1), inputYPos(1)), module,
                                                 M::RETRIG_IN));
    ...
    {
        addChild(
            TextDisplayLight::create(rack::Vec(padFromEdge * 2 + 15, ADSRYPos(i) + 2),
                                     rack::Vec(box.size.x - padFromEdge * 3 - 20, adsrTextH),
                                     module ? &(module->pb[M::A_PARAM + i]->valCache) : nullptr, 13,
                                     NVG_ALIGN_LEFT | NVG_ALIGN_TOP, parameterValueText_KEY()));
    }
    ...
    for (int i = M::A_PARAM; i <= M::R_PARAM; ++i)
    {
        ...
        addParam(rack::createParam<SurgeSmallKnob>(
            rack::Vec(x0, ADSRYPos(ipos) + adsrTextH + padMargin + 3), module, i));
    }
    ...
    for (int i = M::A_TEMPOSYNC; i <= M::R_TEMPOSYNC; ++i)
    {
        ...
        addParam(rack::createParam<SurgeSwitch>(
            rack::Vec(x0 + 2 * portX + 2 * padMargin, ADSRYPos(ipos) + adsrTextH + padMargin + 3),
            module, i));
    }
    ...
    for (int i = M::A_S_PARAM; i <= M::R_S_PARAM; ++i)
    {
        ...
        addParam(rack::createParam<SurgeThreeSwitch>(
            rack::Vec(x0 + 2 * portX + 4 * padMargin + 10,
                      ADSRYPos(ipos) + adsrTextH + padMargin + 1),
            module, i));
        addChild(rack::createLight<rack::SmallLight<rack::GreenLight>>(
            rack::Vec(x0 + 60 + 14 + 10 + 2 * padMargin, ADSRYPos(ipos) + adsrTextH + padMargin),
            module, M::DIGI_LIGHT));
    }

    addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(clockX + padMargin, clockY + padMargin),
                                                 module, M::CLOCK_CV_INPUT));

    addParam(rack::createParam<SurgeSwitchFull>(rack::Vec(box.size.x - modeXOff, modeYPos), module,
                                                M::MODE_PARAM));
}
