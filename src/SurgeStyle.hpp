...
struct SurgeStyle
{
    ...
    static std::string skinAssetDir()
    {
        switch (currentStyle)
        {
        case DARK:
...
inline StyleListener::StyleListener() { SurgeStyle::addStyleListener(this); }
inline StyleListener::~StyleListener() { SurgeStyle::removeStyleListener(this); }
...
static const char *fontFace() { return "res/EncodeSansSemiCondensed-Medium.ttf"; }
static const char *fontFaceCondensed() { return "res/EncodeSansCondensed-Medium.ttf"; }
...
static void drawBlueIORect(NVGcontext *vg, float x0, float y0, float w, float h,
 ...
 */
rack::Vec ioPortLocation(bool input, int ctrl, rack::Rect box, int stackVertically = false)
{ // 0 is L; 1 is R; 2 is gain
    ...
    else
    {
        ...
        float xRes = x0 + ioMargin + padFromEdge + (ctrl * (portX + 4));
        float yRes = orangeLine + 1.5 * ioMargin;
...
void drawLeftRightInputOutputBackground(NVGcontext *vg, rack::Rect box,
                                        std::string inputName = "Input", bool doIn = true,
                                        bool doOut = true)
{
    for (int i = (doIn ? 0 : 1); i < (doOut ? 2 : 1); ++i)
    {
        ...
        SurgeStyle::drawBlueIORect(vg, x0 + ioMargin, orangeLine + ioMargin, ioRegionWidth,
                                   box.size.y - orangeLine - 2 * ioMargin, (i == 0) ? 0 : 1);
        ...
        {
            nvgSave(vg);
            nvgTranslate(vg, x0 + ioMargin + 2, orangeLine + ioMargin * 1.5);
                                      ...
                                      std::string inputName = "Input")
{
    for (int i = 0; i < 2; ++i)
    {
        ...
        SurgeStyle::drawBlueIORect(vg, x0, y0, ioRegionWidth, boxHt, (i == 0) ? 0 : 1);
...
void simpleLabel(NVGcontext *vg, float x, float y, const char *label,
                 int al = NVG_ALIGN_LEFT | NVG_ALIGN_TOP, int fs = 12)
{
    nvgBeginPath(vg);
    nvgFontFaceId(vg, fontId(vg));
    nvgFontSize(vg, fs);
...
/*
** Since styles are dynamic, we want to be able to be notified when they
*change
*/
struct StyleListener
{
