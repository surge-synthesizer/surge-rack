...
struct SurgeRotaryWidget : SurgeModuleWidgetCommon
{
    ...
    void moduleBackground(NVGcontext *vg)
    {
        ...
        std::vector<std::string> lab = {"Horn",    "Rotor",   "Gain",  "Model",
                                        "Doppler", "Tremolo", "Width", "Mix"};
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            if (i % 2)
                xp -= padMargin;
            drawTextBGRect(vg, xp + xText - padFromEdge, y0 + portY + 1.5 * padMargin, w,
                           textHeight);
            if (i % 2)
                y0 += ctrlHeight;
        }
...
SurgeRotaryWidget::SurgeRotaryWidget(SurgeRotaryWidget::M *module) : SurgeModuleWidgetCommon()
{
    setModule(module);
    ...
    bool stacked = false;
    addInput(rack::createInput<rack::PJ301MPort>(ioPortLocation(true, 0, box, stacked), module,
                                                 M::INPUT_L_OR_MONO));
    addInput(rack::createInput<rack::PJ301MPort>(ioPortLocation(true, 1, box, stacked), module,
                                                 M::INPUT_R));
    addParam(rack::createParam<SurgeSmallKnob>(ioPortLocation(true, 2, box, stacked), module,
                                               M::INPUT_GAIN));

    addOutput(rack::createOutput<rack::PJ301MPort>(ioPortLocation(false, 0, box, stacked), module,
                                                   M::OUTPUT_L_OR_MONO));
    addOutput(rack::createOutput<rack::PJ301MPort>(ioPortLocation(false, 1, box, stacked), module,
                                                   M::OUTPUT_R));
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        xp -= 12;
        addParam(rack::createParam<SurgeSmallKnob>(rack::Vec(xp, yp), module, M::FX_PARAM_0 + i));
        addInput(rack::createInput<rack::PJ301MPort>(rack::Vec(xp + portX + padMargin, yp), module,
                                                     M::FX_PARAM_INPUT_0 + i));

        if (i == 0)
        {
            addParam(rack::createParam<SurgeSwitch>(
                rack::Vec(xp + portX * 2 + padMargin * 2, yp + 3), module, M::PARAM_TEMPOSYNC_0));
        ...
        if (i % 2)
            tp += box.size.x / 2;
        addChild(
            TextDisplayLight::create(rack::Vec(tp, yp + portY + 1.5 * padMargin),
                                     rack::Vec(box.size.x / 2 - xText - 2 * padMargin, textHeight),
                                     module ? &(module->pb[i]->valCache) : nullptr, 12,
                                     NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE, parameterValueText_KEY()));
    }

    addInput(rack::createInput<rack::PJ301MPort>(
        rack::Vec(box.size.x / 2 - clockW / 2 + padMargin, clockY + padMargin), module,
        M::CLOCK_CV_INPUT));
}

auto mrotary = addFX(rack::createModel<SurgeRotaryWidget::M, SurgeRotaryWidget>("SurgeRotary"),
                     fxt_rotaryspeaker);
